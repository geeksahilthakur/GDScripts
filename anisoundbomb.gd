...
var explosion_sound_player: AudioStreamPlayer2D  # Reference to the explosion sound player
var blast_sound: AudioStream  # Directly loaded sound for the blast effect
...
func _ready():
	...
	explosion_sound_player = $ExplosionSoundPlayer  # Reference to the existing explosion sound player

	# Load the blast sound effect
	blast_sound = preload("res://sound/medium-explosion-40472.mp3")  # Replace with your actual path
...
func _on_explosion():
	...
	# Play the main explosion sound
	explosion_sound_player.play()

	# Play the additional blast sound directly
	var blast_sound_player = AudioStreamPlayer2D.new()  # Create a new AudioStreamPlayer2D dynamically
	blast_sound_player.stream = blast_sound  # Set the stream to the preloaded sound
	add_child(blast_sound_player)  # Add it as a child to the scene
	blast_sound_player.play()  # Play the sound
